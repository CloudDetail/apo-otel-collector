...
package otlpreceiver // import "github.com/CloudDetail/apo-otel-collector/pkg/receiver/otlpreceiver"
...
import (
	...
	"github.com/CloudDetail/apo-otel-collector/pkg/receiver/otlpreceiver/internal/logs"
	"github.com/CloudDetail/apo-otel-collector/pkg/receiver/otlpreceiver/internal/metrics"
	"github.com/CloudDetail/apo-otel-collector/pkg/receiver/otlpreceiver/internal/trace"
...
func (r *otlpReceiver) getFillProc(host component.Host) fillproc.FillProc {
	if r.cfg.ExtensionID != nil {
		if extension, ok := host.GetExtensions()[*r.cfg.ExtensionID]; ok {
			return extension.(fillproc.FillProc)
		}
		r.settings.Logger.Warn("Unknown fillProc", zap.Any("extensionId", r.cfg.ExtensionID))
	}
	return nil
}
...
func (r *otlpReceiver) startGRPCServer(host component.Host) error {
	...
	if r.nextTraces != nil {
		ptraceotlp.RegisterGRPCServer(r.serverGRPC, trace.New(r.nextTraces, r.obsrepGRPC, r.getFillProc(host)))
	}
...
func (r *otlpReceiver) startHTTPServer(ctx context.Context, host component.Host) error {
	...
	if r.nextTraces != nil {
		httpTracesReceiver := trace.New(r.nextTraces, r.obsrepHTTP, r.getFillProc(host))
