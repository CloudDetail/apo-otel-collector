...
import (
	...
	"github.com/CloudDetail/apo-otel-collector/pkg/receiver/skywalkingreceiver/internal/metrics"
...
type configuration struct {
	...
	ComponmentIdFillProc *component.ID
}
...
type swReceiver struct {
	...
	managementService  *SwManagementServiceServer
...
func (sr *swReceiver) Start(_ context.Context, host component.Host) error {
	if sr.config.ComponmentIdFillProc != nil {
		extension, ok := host.GetExtensions()[*sr.config.ComponmentIdFillProc]
		if !ok {
			return fmt.Errorf("unknown fillProc %q", sr.config.ComponmentIdFillProc)
		}
		sr.traceReceiver.FillProcExtension = extension.(fillproc.FillProc)
	}
...
func (sr *swReceiver) startCollector(host component.Host) error {
	...
	if sr.collectorGRPCEnabled() {
		...
		sr.managementService = &SwManagementServiceServer{
			TraceReceiver: sr.traceReceiver,
		}
		management.RegisterManagementServiceServer(sr.grpc, sr.managementService)
